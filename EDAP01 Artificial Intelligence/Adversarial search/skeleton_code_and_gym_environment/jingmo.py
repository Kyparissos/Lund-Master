...
API_KEY = 'nyckel'
STIL_ID = ["ji8255ba-s"] # TODO: fill this list with your stil-id's
...
def student_move():
   ...
   alpha = float('-inf')
   beta = float('inf')
   return alpha_beta_pruning_minimax(env,depth,alpha,beta,True)

def alpha_beta_pruning_minimax(env,depth,alpha,beta,maximizing_player):
    ...
    if maximizing_player:
      ...
      for child_node in env.available_moves():
         ...
         if done: 
            ...
            return reward
         eval = alpha_beta_pruning_minimax(env_copy, depth-1, alpha, beta, False)
      ...
      for child_node in env.available_moves():
         ...
         if done: 
            if(reward != 1): print('found reward',reward)
            return reward
         eval = alpha_beta_pruning_minimax(env_copy, depth-1, alpha, beta, True)
