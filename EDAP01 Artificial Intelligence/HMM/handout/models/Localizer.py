...
class Localizer:
    ...
    def initialise(self):
        ...
        self.__rs = RobotSimAndFilter.RobotSim(self.__trueState, self.__sm, self.__tm, self.__om)
    ...
    def update(self) -> (bool, int, int, int, int, int, int, int, int, np.array(1)):
        ...
        #
        self.__trueState = self.__rs.new_state()
        self.__sense = self.__rs.new_sensor()
        self.__fVec = self.__HMM.update_f(self.__sense, self.__fVec)
        self.__estimate = self.__sm.state_to_position(np.argmax(self.__fVec))
