...
for kframe = 1:Nframes % First loop over frames
    ...
    for kfold = 1:K % Then loop over the folds
        ...
        estind((kfold-1)*cvhop+1:kfold*cvhop) = [];
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            SEval(kfold,klam) = norm(t(valind) - X(valind,:) * what).^2; % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
            SEest(kfold,klam) = norm(t(estind) - X(estind,:) * what).^2; % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
...
lambdaopt = lambdavec(MSEval == min(MSEval)); % Assign optimal lambda 
