...
for action = 1 : 3 % Evaluate all the different actions (left, forward, right).
    ...
    state_action_feats(2, action) = grid(next_head_loc(1), next_head_loc(2)); % hit the wall or body/ eat the apple


%     if(grid(next_head_loc(1),next_head_loc(2)) == 1) % hit the wall or body
%         state_action_feats(2, action) = -1;
%     else
%         state_action_feats(2,action) = 0;
%     end

%     if (grid(next_head_loc(1),next_head_loc(2)) == -1) % eat the apple
