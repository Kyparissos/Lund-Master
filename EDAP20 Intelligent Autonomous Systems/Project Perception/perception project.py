...
class HSVDetector:
    ...
    def detect(self, hsv_image):
        ...
        """
        self.thresh = cv.inRange(hsv_image, self.hsv_min, self.hsv_max)
        self.thresh = cv.erode(self.thresh, None, iterations=2)
        self.thresh = cv.dilate(self.thresh, None, iterations=2)
        self.contours = cv.findContours(self.thresh.copy(), cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)[-2]
        self.center = None
        self.radius = None
...
def main():
    ...
    """
    hsv_detectors = {
        'blue':   HSVDetector(hsv_min=(110, 100, 20),
                              hsv_max=(130, 255, 255)),

        'orange': HSVDetector(hsv_min=(10, 100, 20),
                              hsv_max=(25, 255, 255)),

        'yellow': HSVDetector(hsv_min=(27, 100, 20),
                              hsv_max=(33, 255, 255)),

        'green':  HSVDetector(hsv_min=(37, 150, 20),
                              hsv_max=(73, 255, 255)),
    }
