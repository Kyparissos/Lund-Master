...
for i = 1 : nbr_trials
    ...
    classification_data = trainSimpleCNN(X_train, Y_train);
    ...
    for j = 1 : nbr_test_examples
        ...
        predictions_test(j) = predictSimpleCNN(X_test(:, j), classification_data);
    end
    ...
    for j = 1 : nbr_train_examples
        % YOU SHOULD IMPLEMENT THE FUNCTION classify!
        predictions_train(j) = predictSimpleCNN(X_train(:, j), classification_data);
    end
...
%     ave_test_err = mean(err_rates_test(:,1));
%     ave_train_err = mean(err_rates_train(:,1));

% test
% im1 = reshape(X(:,10),[19,19]);
% im2 = reshape(X(:,19),[19,19]);
% imagesc(im2);
    ...
    % transppose matrix
%     new_X_train = transpose(X_train);
%     new_X_test = transpose(X_test);
%     new_Y_train = transpose(Y_train);
%     new_Y_test = transpose(Y_test);
% 
%     % modelfit
%     tree_model = fitctree(new_X_train,new_Y_train);
%     svm_model = fitcsvm(new_X_train,new_Y_train);
%     nn_model = fitcknn(new_X_train,new_Y_train);
%     
%     % Next, let's use our trained model to classify the examples in the 
%     % test data. You should look up the function "predict" in Matlab!
%     % (don't forget: transpose as necessary, both for X and Y)
%     predictions_test_tree = tree_model.predict(new_X_test);
%     predictions_test_svm = svm_model.predict(new_X_test);
%     predictions_test_nn = nn_model.predict(new_X_test);
%    
%     % We can now proceed to computing the respective error rates.
%     pred_test_diff_tree = predictions_test_tree - new_Y_test;
%     pred_test_diff_svm = predictions_test_svm - new_Y_test;
%     pred_test_diff_nn = predictions_test_nn - new_Y_test;
...
%     predictions_train_tree = tree_model.predict(new_X_train);
%     predictions_train_svm = svm_model.predict(new_X_train);
%     predictions_train_nn = nn_model.predict(new_X_train);
%     pred_train_diff_tree = predictions_train_tree - new_Y_train;
%     pred_train_diff_svm = predictions_train_svm - new_Y_train;
%     pred_train_diff_nn = predictions_train_nn - new_Y_train;
