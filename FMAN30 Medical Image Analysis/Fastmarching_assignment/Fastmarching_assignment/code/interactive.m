...
I(I > 0) = (I(I > 0) - min(I(I > 0), [], 'all')) / (max(I(I > 0), [], 'all') - min(I(I > 0), [], 'all'));
I = imadjust(I);
I = imgaussfilt(I,2);
J = grayconnected(I, GUI.YSeed, GUI.XSeed);
intensityatseed = GUI.IM(GUI.YSeed,GUI.XSeed);
ed = edge(I,"canny");
SE = strel('disk',2);
ed = imdilate(ed,SE);

GUI.SPEED = GUI.Slider1*J;
GUI.SPEED(ed==1) = min(GUI.SPEED(:));
GUI.SPEED(GUI.SPEED<1e-2) = 1e-6;
%% 
% intensityatseed = GUI.IM(GUI.YSeed,GUI.XSeed);
% GUI.SPEED = exp(-(GUI.IM - intensityatseed).^2/(2*GUI.Slider1^2));
% im_edge = edge(GUI.IM,'Canny',GUI.Slider3);
% GUI.SPEED(im_edge==1) = min(GUI.SPEED(:));
...
% 
% % Normalize image
% im(im > 0) = (im(im > 0) - min(im(im > 0), [], 'all')) / (max(im(im > 0), [], 'all') - min(im(im > 0), [], 'all'));
% 
% % Enhance contrast
% im = imadjust(im, [0.1, 0.9]);
% im_int = im.^(GUI.Slider2*0.1);   
% 
% % Blur the image with a Gaussian filter
% blur = GUI.Slider2*imgaussfilt(im_int, GUI.Slider2*0.03 + 0.001);
% 
% % Find region with similar intensity to the seed
% conn = grayconnected(im, GUI.YSeed, GUI.XSeed, 0.1);
% 
% % Find large edges
% edge = imfilter(imgaussfilt(im, 2.7), fspecial('laplacian'), 'same');
% edge = bwareafilt(imbinarize(edge), [4e2, 1e6]);
% edge = imdilate(edge, strel('disk',double(GUI.Slider3*100)));
% 
% GUI.SPEED = GUI.Slider1*conn;
% 
% % Make sure that the speed of the object at the seed is larger than its bounds
% if GUI.SPEED(GUI.YSeed,GUI.XSeed) > mean(GUI.SPEED, 'all')
%     GUI.SPEED = imcomplement(GUI.SPEED);
% end

% Normalize, apply squared cos and set low values to zero
% GUI.SPEED = (GUI.SPEED - min(GUI.SPEED, [], 'all')) / (max(GUI.SPEED, [], 'all') - min(GUI.SPEED, [], 'all'));
% GUI.SPEED = cos(GUI.SPEED).^2;
% GUI.SPEED(GUI.SPEED.^2 < 0.4) = 1e-6;
