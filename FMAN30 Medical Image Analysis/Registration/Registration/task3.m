...
for i = 1:length(xs)
    ...
    for j = 1:length(x)
        eps{i,j} = y(:,j) - (R*x(:,j) + t);
        epsn_manual{i,j} = norm(eps{i,j})^2;
        sum_eps = sum_eps + norm(eps{i,j})^2;
    end
    ...
    sum_eps = 0;
    for j = 1:length(x)
        epsa{i,j} = y(:,j) - (Rs{i}*x(:,j) + ts{i});
        epsn_auto{i,j} = norm(epsa{i,j})^2;
        sum_eps = sum_eps + norm(epsa{i,j})^2;
    end
    eps_auto = sqrt((1/(N-1))*sum_eps);

    Eps_manual{i,1} = eps_manual;
    Eps_auto{i,1} = eps_auto;
    evaluation{i,1} = (eps_auto <= eps_manual + 5);
end
%% 
im=1;
p1x=[283, 217]; p1y=[290, 206];
p2x=[349, 391];p2y=[364,374];
p3x=[77, 321];p3y=[87, 323];
p4x=[496, 296];p4y=[506, 273];
manual_points_x{im} = [p1x; p2x; p3x; p4x]';
manual_points_y{im} = [p1y; p2y; p3y; p4y]';

im=6;
p1x=[332,423];p1y=[355, 402];
p2x=[267, 153];p2y=[253, 145];
p3x=[122, 364];p3y=[136, 372];
p4x=[208, 52];p4y=[192, 52];
manual_points_x{im} = [p1x; p2x; p3x; p4x]';
manual_points_y{im} = [p1y; p2y; p3y; p4y]';
%% 
 x = xs{6};
    ...
    for j = 1:length(x)
        eps{6,j} = y(:,j) - (R*x(:,j) + t);
        epsn_manual{i,j} = norm(eps{6,j})^2;
        sum_eps = sum_eps + norm(eps{6,j})^2;
    end
    eps_manual = sqrt((1/(N-1))*sum_eps);
    ...
    sum_eps = 0;
    for j = 1:length(x)
        epsa{6,j} = y(:,j) - (Rs{6}*x(:,j) + ts{6});
        epsn_auto{6,j} = norm(epsa{6,j})^2;
        sum_eps = sum_eps + norm(epsa{6,j})^2;
    end
    eps_auto = sqrt((1/(N-1))*sum_eps);

    Eps_manual{6,1} = eps_manual;
    Eps_auto{6,1} = eps_auto;
%% 
