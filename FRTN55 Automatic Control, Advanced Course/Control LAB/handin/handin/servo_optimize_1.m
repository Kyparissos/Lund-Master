...
%%% Task 0: Fill in your name(s) here: Zuoyi Yu  Jingmo Bai
...
Q = [0.45 0.1];        % Define static Q parameter (1x2 vector)
Gzw = Tzw + Tzu*Q*Tyw;  % Calculate closed loop
% figure(1);clf
% plotspecs(Gzw)          % Check result against Lab 1 time-domain specs

% A: It is impossible to meet the constraints with a constant Q parameter.
% A constant Q parameter can not provide the flexibility needed to
% simultaneously optimize all the objectives and meet the constraints.
...
cvx_begin
  ...
  minimize sum_square(Gzw_stepresp(1,3,:))

  subject to:

  %%% Task 6: Add the correct constraint on the response from n to u below
  
  sum_square(Gzw_stepresp(2,2,:))*h <= 1;
  
  %% Task 7: Add the correct time-domain constraints below
  
  %Upper and lower constraints on step response from r to z0:
  constr.Gz0r_lower <= squeeze(Gzw_stepresp(1,1,:)) <= constr.Gz0r_upper;
  
  %Upper and lower constraints on step response from r to u:
  constr.Gur_lower <= squeeze(Gzw_stepresp(2,1,:)) <= constr.Gur_upper;

  %Upper and lower constraints step response from d to z0:
  constr.Gz0d_lower <= squeeze(Gzw_stepresp(1,3,:)) <= constr.Gz0d_upper;

  %Upper and lower constraints on step response from d to u:
  constr.Gud_lower <= squeeze(Gzw_stepresp(2,3,:)) <= constr.Gud_upper;

  %% Task 8: Experiment with a constrint on Ms and comment below
 
  abs(1 - Gzw_freqresp(1,2,:)) <= 1.6;
%   abs(Gzw_freqresp(1,3,:)) <= 1.6;


  % The optimization is feasible until Ms is lower than 1.4. 
  % Smaller MS could cause disturbance d having bigger influence to
  % output z0, making the system less robost to disturbance.
  
  %% Task 9: Experiment with Nq and comment on the results here:

  % The smallest Nq that can satisfy all of the stated constraints we found
  % is 20. With this setting, we can observe more oscillations in the time responses 
  % and cvx optval increased from around 16 to around 26.
  % There is almost no difference when we change Nq to 60 and 120. Larger
  % values will cause overfitting, so it is pointless to keep increasing
  % the number of Nq.
