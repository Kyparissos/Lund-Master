...
Phi_imu = [1 0;Ts 1];      % Assignment 5
Gam_imu = 0;      % Assignment 5
Cd_imu  = [1 0; 0 1];      % Assignment 5
Dd_imu  = 0;      % Assignment 5
Gd_imu  = [Ts;0];      % Assignment 5

R1_imu  = 1;      % Assignment 5
...
% m1 = 100;   % Assignment 11
% m2 = 0.00005;   % Assignment 11
% m3 = 100;   % Assignment 11
...
% reg = ss(Phi_lqg, Gam_lqg, C_lqg, D_lqg, Ts);
% % Cloped loop
% Gcl = feedback(sysd, reg);
% Gcl = ss(Gcl.A, Gcl.B, [Gcl.C; zeros(size(K)), -K], 0, Ts);
% % Simulation
% [Y,T,X] = initial(sysd,[1 -1 0 0],0:Ts:20);
% subplot(411)
% plot(T, X(:,1)); hold on; plot(T, X(:,3),'--'); grid on
% legend('x1','x1hat'); ylabel('x1, x1hat')
% %% Assignment 12
